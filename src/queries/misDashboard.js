...
export async function getTopCustomers(connection, type = "YEAR", filterYear, filterBuyer, filterMonth) {
    let result;
    if (type === "YEAR") {
        const sql = ` SELECT SUM(STAFF) STAFF,SUM(LABOUR) LABOUR,SUM(STAFF)+SUM(LABOUR) TOTAL,COMPCODE FROM (SELECT CASE WHEN CC.BANDID = 'STAFF' THEN A.NETPAY ELSE 0 END STAFF,
CASE WHEN CC.BANDID <> 'STAFF' THEN A.NETPAY ELSE 0 END LABOUR,A.COMPCODE  FROM HPAYROLL A
JOIN HREMPLOYMAST AA ON A.EMPID = AA.IDCARDNO
...
export async function getLoss(connection, type = "YEAR", filterYear, filterBuyer, filterMonth) {
    ...
    if (type === "YEAR") {
        const sql =
            `SELECT SUM(MALE) MALE,SUM(FEMALE) FEMALE,SUM(MALE)+SUM(FEMALE) TOTAL,COMPCODE  FROM (
SELECT CASE WHEN AA.GENDER = 'MALE' THEN A.OTWAGES ELSE 0 END MALE,
CASE WHEN AA.GENDER = 'FEMALE' THEN A.OTWAGES ELSE 0 END FEMALE,A.COMPCODE  FROM HPAYROLL A
JOIN HREMPLOYMAST AA ON A.EMPID = AA.IDCARDNO
WHERE A.PAYPERIOD = '${lstMnth}') group by compcode`
        console.log(sql, 'sql');
        result = await connection.execute(sql)
